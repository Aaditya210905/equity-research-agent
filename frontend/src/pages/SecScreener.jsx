...
export default function SecScreener() {
  const { token } = useAuth();
  ...
  const toggleCategory = (ticker, kind) => {
  ...
  const fetchFolders = async () => {
    try {
      const res = await fetch('http://localhost:8000/sec/filings');
      if (res.ok) {
  ...
  const handleSearch = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`http://localhost:8000/sec/search?query=${query}`);
  ...
  const handleFetch = async (company) => {
    setLoading(true);
    if (!activeTickers.includes(company.ticker)) {
      setActiveTickers(prev => [company.ticker, ...prev]);
    }
    try {
      const res = await fetch(`http://localhost:8000/sec/filings/fetch`, {
        method: 'POST',
        headers: { 
  ...
  return (
    <div>
      <div className="top-bar">
        <div>
          <h2>SEC EDGAR Screener</h2>
          <p style={{ color: 'var(--text-secondary)' }}>Search and download 10-K and 10-Q filings from the US SEC</p>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: '30px' }}>
        ...
        <div style={{ flex: 1 }}>
          <div className="glass-card">
            <form onSubmit={handleSearch} style={{ display: 'flex', gap: '10px', marginBottom: '20px' }}>
              <input 
                type="text" 
                placeholder="Search US ticker (e.g. AAPL, MSFT)..." 
                value={query}
            ...
            {loading && <div style={{ color: 'var(--text-secondary)' }}>Searching/Fetching...</div>}
              ...
              {searchResults.map((res, i) => (
                <div key={i} style={{ padding: '15px', background: 'rgba(255,255,255,0.05)', borderRadius: '8px', marginBottom: '10px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div>
                    ...
                    <span style={{ fontSize: '0.8rem', color: 'var(--accent)' }}>Ticker: {res.ticker} | CIK: {res.cik}</span>
                  </div>
        ...
        {/* Right panel: Downloaded Folders */}
        <div style={{ flex: 2 }}>
            <div className="glass-card">
              <h3 style={{ marginBottom: '20px' }}><Globe size={20} style={{ verticalAlign: 'middle', marginRight: '10px' }}/> Fetched Documents</h3>
              
              {fetchedFolders.filter(f => activeTickers.includes(f.symbol)).length === 0 ? (
                <p style={{ color: 'var(--text-secondary)' }}>No filings fetched yet. Search for a US company and click "Fetch Filings" to view documents here.</p>
              ) : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                  {fetchedFolders.filter(f => activeTickers.includes(f.symbol)).map((folder, i) => {
                    ...
                    return (
                      <div key={i} style={{ border: '1px solid var(--border)', borderRadius: '8px', padding: '15px' }}>
                        <h4 style={{ marginBottom: '15px', color: 'var(--text-primary)' }}>{folder.name} <span style={{ fontSize: '0.8rem', color: 'var(--accent)', fontWeight: 'normal', marginLeft: '10px' }}>CIK: {folder.scripCode}</span></h4>
                        
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                          {Object.entries(groupedFiles).map(([kind, files]) => {
                            const catKey = `${folder.symbol}-${kind}`;
                            const isExpanded = expandedCategories[catKey];
                            
                            return (
                              <div key={kind}>
                                <button 
                                  onClick={() => toggleCategory(folder.symbol, kind)}
                                  ...
                                  <ul style={{ listStyle: 'none', padding: '10px 0 10px 25px', margin: 0, borderLeft: '1px solid rgba(255,255,255,0.1)', marginLeft: '8px' }}>
                                    {files.map((file, j) => (
                                      <li key={j} style={{ padding: '6px 0' }}>
                                        <a 
                                          href={`http://localhost:8000/sec/file/${encodeURIComponent(file.relativePath)}`}
                                          target="_blank"
                                          rel="noreferrer"
                                          style={{ textDecoration: 'none', background: 'none', border: 'none', color: 'var(--text-secondary)', cursor: 'pointer', textAlign: 'left', display: 'flex', alignItems: 'flex-start', gap: '10px', fontSize: '0.9rem' }}
                                        >
